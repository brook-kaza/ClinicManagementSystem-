...
import { DollarSign, Plus, CheckCircle, Clock, CreditCard, Receipt, FileText } from 'lucide-react';
...
const PatientBilling = ({ patientId }) => {
    ...
    // Form States
    const [newInvoice, setNewInvoice] = useState({ description: '', total_amount: '' });
    ...
    const handleCreateInvoice = async (e) => {
        e.preventDefault();
        try {
            const res = await api.post(`/billing/patients/${patientId}/invoices`, {
                description: newInvoice.description,
                total_amount: parseFloat(newInvoice.total_amount)
            });
            setInvoices([res.data, ...invoices]);
            setNewInvoice({ description: '', total_amount: '' });
    ...
    const handleLogPayment = async (e) => {
        ...
        try {
            ...
            // Optimistically update invoice list to reflect new payment and status
            fetchInvoices();
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="grid grid-cols-3 gap-4 max-w-sm bg-zinc-50 border border-zinc-100 rounded-xl p-3">
                                            <div>
                                                ...
                                                <p className="font-bold text-zinc-800">${invoice.total_amount.toFixed(2)}</p>
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold text-rose-600">${balance.toFixed(2)}</p>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            {invoice.payments.map((p, idx) => (
                                                <div key={idx} className="flex items-center justify-between bg-zinc-50/50 px-3 py-2 rounded-lg text-sm border border-zinc-100 group">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="font-semibold text-zinc-700">${p.amount_paid.toFixed(2)}</span>
            ...
            {showInvoiceModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl relative">
                        <h3 className="text-xl font-bold font-heading mb-4 text-zinc-800">Generate New Invoice</h3>
                        <form onSubmit={handleCreateInvoice} className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Description / Treatment</label>
                                <input type="text" required value={newInvoice.description} onChange={e => setNewInvoice({ ...newInvoice, description: e.target.value })} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none" placeholder="e.g. Root Canal Therapy" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Total Amount ($)</label>
                                <input type="number" required min="0" step="0.01" value={newInvoice.total_amount} onChange={e => setNewInvoice({ ...newInvoice, total_amount: e.target.value })} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none" placeholder="0.00" />
                            </div>
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl relative">
                        ...
                        <div className="bg-indigo-50 rounded-xl p-4 mb-6 border border-indigo-100 flex justify-between items-center">
                            ...
                            <span className="text-lg font-black text-indigo-600">
                                ${(selectedInvoice.total_amount - selectedInvoice.payments.reduce((s, p) => s + p.amount_paid, 0)).toFixed(2)}
                            </span>
                        </div>

                        <form onSubmit={handleLogPayment} className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Payment Amount ($)</label>
